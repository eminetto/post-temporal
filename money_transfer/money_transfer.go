...
func MoneyTransfer(ctx workflow.Context, input payment.Details) (string, error) {
	// RetryPolicy specifies how to automatically handle retries if an Activity fails.
	retrypolicy := &temporal.RetryPolicy{
		InitialInterval:        time.Second,
		BackoffCoefficient:     2.0,
		MaximumInterval:        100 * time.Second,
		MaximumAttempts:        30, // 0 is unlimited retries
		NonRetryableErrorTypes: []string{"InvalidAccountError", "InsufficientFundsError", "OverLimitError"},
	}
	...
	withdrawErr := workflow.ExecuteActivity(ctx, withdraw.Withdraw, input).Get(ctx, &withdrawOutput)
	...
	depositErr := workflow.ExecuteActivity(ctx, deposit.Deposit, input).Get(ctx, &depositOutput)
	...
	if depositErr != nil {
		...
		refundErr := workflow.ExecuteActivity(ctx, refund.Refund, input).Get(ctx, &result)
